...
void cpumm(D_TYPE *a, D_TYPE *b, D_TYPE *c)
{
  ...
  for(i = 0; i < ARRAY_SIZE; i+=n)
  {
    for(j = 0; j < ARRAY_SIZE; j+=n)
    {
      for(k = 0; k < ARRAY_SIZE; k+=n)
      {        
          for(ii = i; ii < i+n; ii++)
            for(jj = j; jj < j+n; jj++)
              for(kk = k; kk < k+n; kk++)
  ...
  fprintf(stderr, "%d,%dx%dx%d, %f, %0.2f\n", sizeof(D_TYPE),ARRAY_SIZE, ARRAY_SIZE, ARRAY_SIZE, total_time* 1e3, flops * 1e-9);
}
